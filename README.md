...
演示阶段采用固定五候选的加权锚点识别。每张 `data/artifacts/*.json` 卡片包含 `recognition_anchors` 和 `recognition_conflicts`：权重 5 表示决定性独有锚点，权重 4 表示强锚点，权重 3 表示辅助锚点；明显冲突的器型结构会降低候选优先级。屏幕边框、摩尔纹、反光、偏色、过曝和轻微模糊不会直接导致 `unknown`，但屏幕文字和标题不能作为识别证据。器型和结构优先于颜色，尤其三个青铜器不能只凭青铜色、绿色锈色、圆腹或三足判断：盘龙钮带盖铜盉重点看虎首管状流、盘龙钮和对侧鋬；邓公簋重点看左右对称的兽首衔环耳、带盖圆腹和喇叭形盖钮；束腰垂鳞纹升鼎重点看高耸外撇立耳、口沿附兽和三只粗壮蹄足。
...
馆级资料放在 `data/museum/pingdingshan_museum.json`，由 `museum.py` 加载。当前资料使用平顶山市文化广电和旅游局、平顶山市人民政府地方史志等官方公开来源，包含地址、电话、常规开放时间、开馆日期、建筑与馆藏规模、主要陈列、单位职责和镇馆之宝口径。

当用户询问“几点开馆”“周一闭馆吗”“镇馆之宝是什么”“馆里有多少藏品”“博物馆在哪里”等问题时，`router.py` 会注入馆级资料，不再要求用户先拍摄文物。馆级问题不会错误继承设备当前文物；如果用户同时明确提到文物，例如“应国玉鹰为什么是镇馆之宝”，则会同时注入馆级资料和对应文物知识卡。

开放时间属于可能变化的访客信息。资料卡只把周二至周日 9:00—17:00、周一闭馆作为常规安排；对于今天、法定节假日、临时闭馆或延时开放，回答会提示以馆方当天公告或电话确认为准。当前没有可靠资料支持的门票、预约、停车、停止入馆时间和公交细则也不会由模型自行补全。

当前官方地方大事记将玉鹰称为市博物馆镇馆之宝，因此可以回答“应国玉鹰”。另外四件已配置文物只称为重点讲解文物或代表性馆藏，不统一称为镇馆之宝。
